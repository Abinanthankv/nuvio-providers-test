...
/**
 * Finds a working Isaidub domain
 */
...
async function resolveImdbId(imdbId) {
    try {
        ...
        if (movie) {
            ...
            console.log(`[Isaidub] IMDb ${imdbId} -> Movie: "${info.title}" (${info.year || 'N/A'})`);
            return info;
        }
        if (tv) {
            const info = { title: tv.name, year: (tv.first_air_date || '').split('-')[0] };
            console.log(`[Isaidub] IMDb ${imdbId} -> TV: "${info.title}" (${info.year || 'N/A'})`);
            return info;
        }
        console.log(`[Isaidub] IMDb ${imdbId} not found on TMDB`);
        return null;
    } catch (error) {
        console.error(`[Isaidub] Error resolving IMDb ID ${imdbId}:`, error.message);
        return null;
    }
}
...
async function extractFinalDownloadUrl(downloadPageUrl) {
    ...
    try {
        ...
        $('a').each((i, el) => {
            ...
            if (href && href.startsWith('http') && !href.startsWith('#')) {
                if (text.includes('download') || text.includes('server') || 
                    href.includes('dubmv.') || href.includes('dubpage.') || 
                    href.includes('dubshare.') || href.includes('onestream.today')) {
        ...
        if (downloadLinks.length > 0) {
            ...
            const needsExtraction = downloadUrl.includes('dubpage.') || 
...
async function getStreams(tmdbId, mediaType, season, episode) {
    ...
    try {
        ...
        if (imdbMatch) {
            ...
            if (!mediaInfo) {
                console.log(`[Isaidub] IMDb resolve failed for "${tmdbId}", using as-is`);
