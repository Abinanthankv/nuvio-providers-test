...
function getReadyDomain() {
  return __async(this, null, function* () {
    ...
    const results = yield Promise.allSettled(
      POTENTIAL_DOMAINS.map((domain) => __async(this, null, function* () {
        yield fetchWithTimeout(domain, { method: "GET" }, 5e3);
        return domain;
      }))
    );
...
function calculateTitleSimilarity(title1, title2) {
  ...
  const union = /* @__PURE__ */ new Set([...wordSet1, ...wordSet2]);
  if (union.size === 0)
    return 0;
...
\u{1F4FC}: ${displayTitle}${yearStr} ${quality}
...
function extractDirectVideoUrls(html) {
  ...
  for (const [, script] of scripts) {
    if (!script.includes("eval(function(p,a,c,k"))
      continue;
...
function smartExtract(_0) {
  return __async(this, arguments, function* (url, depth = 0, maxDepth = 2, visited = /* @__PURE__ */ new Set()) {
    ...
    const streams = [];
    try {
      const timeout = depth === 0 ? 8e3 : 5e3;
      const res = yield fetchWithTimeout(url, { headers: HEADERS, redirect: "follow" }, timeout);
      ...
      if (depth === 0)
        console.error(`[TamilMV] Extract error: ${e.message}`);
    }
    return streams;
  });
}
function extractMagnetLinks(topicUrl) {
  return __async(this, null, function* () {
    try {
      console.log(`[TamilMV] Scanning topic page for magnet links: ${topicUrl}`);
      const response = yield fetchWithTimeout(topicUrl, { headers: HEADERS }, 8e3);
      if (!response.ok)
        return [];
      ...
      while ((match = magnetRegex.exec(html)) !== null) {
        ...
        if (dnMatch) {
          ...
          if (/2160p|4K/i.test(dn))
            quality = "4K";
          else if (/1080p/i.test(dn))
            quality = "1080p";
          else if (/720p/i.test(dn))
            quality = "720p";
          else if (/480p/i.test(dn))
            quality = "480p";
...
function extractTopicDirectLinks(topicUrl) {
  return __async(this, null, function* () {
    const streams = [];
    try {
      const res = yield fetchWithTimeout(topicUrl, { headers: HEADERS }, 8e3);
      if (!res.ok)
        return streams;
      const html = yield res.text();
      ...
      for (const url of fileHostUrls.slice(0, 5)) {
        const extracted = yield smartExtract(url, 0, 1, visited);
...
function extractHomepageWatchLinks(html) {
  ...
  while ((match = wLinkRegex.exec(html)) !== null) {
    ...
    if (!watchUrl || results.some((r) => r.watchUrl === watchUrl))
      continue;
  ...
  $('a[href*="/forums/topic/"]').each((i, el) => {
    ...
    if (!href)
      return;
    if (results.some((r) => r.watchUrl === href || r.topicUrl === href))
      return;
    ...
    try {
      ...
      if (!bestMatch) {
        ...
        for (const match of topResults) {
          const magnets = match.topicUrl ? yield extractMagnetLinks(match.topicUrl) : [];
          for (const magnet of magnets) {
            if (finalStreams.some((s) => s.url === magnet.url))
              continue;
      ...
      try {
        ...
        if (watchUrl && !isTopicUrl) {
          ...
          const extracted = yield smartExtract(watchUrl, 0, 2, visited);
          for (const result of extracted) {
            if (result.url && !finalStreams.some((s) => s.url === result.url)) {
              const quality = bestMatch.title.includes("2160p") || bestMatch.title.includes("4K") ? "4K" : bestMatch.title.includes("1080p") ? "1080p" : bestMatch.title.includes("720p") ? "720p" : result.quality || "HD";
              let size = "Unknown";
        ...
        if (bestMatch.topicUrl) {
          ...
          const magnets = yield extractMagnetLinks(bestMatch.topicUrl);
          for (const magnet of magnets) {
            if (finalStreams.some((s) => s.url === magnet.url))
              continue;
          ...
          const directStreams = yield extractTopicDirectLinks(bestMatch.topicUrl);
